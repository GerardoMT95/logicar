	...
	**/
	...
	switch($stato){
		...
		case 1: // Richiesto non approvato
				...
				break;
		case 2:
		case 4: // Approvato non caricato
			
				$query="UPDATE `ric_richieste_esami` SET stato=5 WHERE id='$ric_codi'";
				if(mysql_query($query, $db)){
					include 'require_conn.php';

				   try { 
				   $conn = new PDO("mysql:host=$servername;dbname=$dbname", $username, $password);
				   $statement = $conn->prepare("DELETE FROM `viaggi_etichette` WHERE viaggio='$viaggio' AND etichetta='$ric_codi'");
				   $statement->execute();
				...
				echo 'Richiesta correttamente Scaricata';
				}
				break;
		case 3: echo 'Richiesta cancellata dal Reparto';
				break;
		case 5: echo 'Richiesta già Scaricata';
				break;
		default:
